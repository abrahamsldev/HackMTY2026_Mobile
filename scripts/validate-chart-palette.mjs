/**
 * Vendored from the `dataviz` skill's `validate_palette.js` (self-contained, no
 * dependencies). It is the gate `tests/chart-palette.test.mjs` runs every chart
 * palette in `src/features/accessibility/theme.ts` through, so a palette change
 * is checked by computation — never by eye.
 *
 * CLI still works when invoked by its original name; as a module it exports
 * `validate` (categorical) and `validateOrdinal` (single-hue ramps).
 *
