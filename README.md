...
- Los botones producen la acción oficial con `name`, `surfaceId`, `sourceComponentId`, `timestamp` y el `context` declarado resuelto contra el modelo de datos. El agente toma la identidad de la sesión y el MCP limita la ejecución al contrato registrado. Además de consultas, los formularios confirmados pueden guardar presupuestos y metas, transferir entre cuentas o a un beneficiario y pagar una tarjeta cuando están desplegadas las migraciones y credenciales de acciones.
